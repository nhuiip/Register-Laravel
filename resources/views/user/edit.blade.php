...
<div class="card shadow mb-4">
    <div class="card-body">
        <form class="form-horizontal" method="POST" action="{{ route('user.update', $id) }}" novalidate>
            {{ csrf_field() }}
            {{ method_field('PUT') }}
                    ...
                    <div class="form-group col-sm-6{{ $errors->has('name') ? ' has-error' : '' }}">
                        ...
                        <div class="col-sm-12">
                            <select name="position_id" id="position_id"
                                ...
                                <option value="{{ $value->position_id }}" @if ($value->position_id == $position_id)
                                        {{ selected }} @endif>{{ $value->position_name }}</option>
                                @endforeach
                            </select>
